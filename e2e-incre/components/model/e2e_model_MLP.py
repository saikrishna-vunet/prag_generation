...
class E2EMLPModel(E2ESeq2SeqModel):
    ...
    def forward(self, datum):
        ...
        # Embedding lookup
        encoder_input_embedded = self.embedding_lookup(
            batch_x_var)  # SL x B x E
        encoder_input_embedded = self.embedding_dropout_layer(
        ...
        if use_teacher_force:
            logits = self.decode_teacher(
                batch_y_var, encoder_hidden, encoder_outputs)
        else:
            logits = self.decode_dynamic(
                batch_y_var, encoder_hidden, encoder_outputs)
    ...
    def predict_dis(self, input_var, input_dis, alpha=1.0):
        ...
        cur_m = cuda_if_gpu(torch.FloatTensor(
            [real_simi / (real_simi + cur_simi), cur_simi / (real_simi + cur_simi)]))
        ...
        while (curr_token_id != EOS_ID and curr_dec_idx <= self.max_tgt_len):
            ...
            dis_o, dis_dec_h, dis_a = self.decoder(
                prev_y, dis_dec_h, dis_enc_o)
            #print(decoder_output.shape, dis_o.shape)
            s0_p = torch.cat([decoder_output.unsqueeze(-1),
                             dis_o.unsqueeze(-1)], dim=-1)
            gold_p = decoder_output + torch.log(cur_m[0][0])
            dis_p = dis_o + torch.log(cur_m[0][1])
            s0_tmp = torch.cat(
                [gold_p.unsqueeze(-1), dis_p.unsqueeze(-1)], dim=-1)
    ...
    def predict_beam(self, input_var, beam_size=5, length_normalization_factor=0.0, length_normalization_const=5.):
        ...
        prev_y = self.embedding_mat(dec_input_var)
        decoder_output, dec_hidden, decoder_attention = self.decoder(
            prev_y, dec_hidden, encoder_outputs)  # decode the logprobs, words
        ...
        for i in range(self.max_tgt_len):
            ...
            for input_feed_, state_feed_ in zip(input_feed, state_feed):
                dec_input_var = cuda_if_gpu(
                    Variable(torch.LongTensor([input_feed_])))
                prev_y = self.embedding_mat(dec_input_var)
                decoder_output, dec_hidden, decoder_attention = self.decoder(
                    prev_y, state_feed_, encoder_outputs)
                logprobs, words = decoder_output.data.topk(beam_size + 1)
                logprobs, words = logprobs[0], words[0]
                # print("dec_input_var, words", words )
                words_.append(words), logprobs_.append(
                    logprobs), states_.append(dec_hidden)
            idx = 0
